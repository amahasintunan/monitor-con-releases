...
is_jdk21() {
  ...
  if echo "$version" | grep -qE '"(2[1-9]|[3-9][0-9])\.'; then
    return 0  # JDK 21+
  fi
  # Also accept 17-20 (close enough — may still work)
...
if [ -z "$JAVA_CMD" ]; then
  ...
  echo "  2. Set JAVA_HOME in your shell:  export JAVA_HOME=/path/to/jdk"
  echo "  3. Edit jmonitor-server.ini and set JAVA_HOME there"
  echo ""
  exit 1
fi
...
exec "$JAVA_CMD" -cp "$JAR_FILE" -Xms128m -Xmx128m com.jmonitor.server.JMonitorServer "$@"
